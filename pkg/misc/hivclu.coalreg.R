...
cr.master.ex3.runcoalreg.using.TYPE<- function()
{
	require(coalreg)
	require(viridis)
	#
	#	run coalreg (no extra args to trf.lasso)
	#
	indir	<- '~/Dropbox (Infectious Disease)/OR_Work/2017/2017_coalregression/master_examples'
	infiles	<- data.table(F=list.files(indir, pattern='m3.RR5.n150_seed123_rep[0-9]+.nwk',full.names=TRUE))	
	infiles[, {
				...
				set(phi, NULL, 'TYPE', phi[,factor(TYPE,levels=c('I0','I1'))])
				rownames(phi)	<- phi[, TAXA]
				set(phi, NULL, 'TAXA', NULL)
				tmp		<- data.matrix(phi)
				fit 	<- trf.lasso(dph, tmp, trf_names = c( 'TYPE'), aoi_names = c( 'TYPE' ), lasso_threshold= 9, method = 'Nelder-Mead')	
				fci 	<- fisher.ci(fit)	 
				pci 	<- prof.ci(fit, fci  ) 
				tmp		<- file.path(dirname(F), gsub('\\.nwk','_coalreg_using_TYPE.rda',basename(F)))
				save( fit, fci, pci, file=tmp)
			}, by='F']	
	#
	#	summarize estimated coefficients
	#
	infiles	<- data.table(F=list.files(indir, pattern='m3.RR5.n150_seed123_rep[0-9]+_coalreg_using_TYPE.rda',full.names=TRUE))
	res		<- infiles[, {
	...
	infiles[, {
				...
				fci 	<- fisher.ci(fit)	 
				pci 	<- prof.ci(fit, fci  ) 
				tmp		<- file.path(dirname(F), gsub('\\.nwk','_coalreg_using_TYPE_BFGSargs.rda',basename(F)))
				save( fit, fci, pci, file=tmp)
			}, by='F']	
	#
	#	summarize estimated coefficients
	#
	infiles	<- data.table(F=list.files(indir, pattern='m3.RR5.n150_seed123_rep[0-9]+_coalreg_using_TYPE_BFGSargs.rda',full.names=TRUE))
	res		<- infiles[, {
				#F	<- '/Users/Oliver/Dropbox (Infectious Disease)/OR_Work/2017/2017_coalregression/master_examples/m3.RR5.n150_seed123_rep99_coalreg_using_TYPE.rda'
				load(F)
				list(	THETA= names(fit$bestfit$par),
	...
	infiles	<- data.table(F=list.files(indir, pattern='m3.RR5.n150_seed123_rep[0-9]+_coalreg_using_TYPE_BFGSargs_s50.rda',full.names=TRUE))
	res		<- infiles[, {
				#F	<- '/Users/Oliver/Dropbox (Infectious Disease)/OR_Work/2017/2017_coalregression/master_examples/m3.RR5.n150_seed123_rep99_coalreg_using_TYPE.rda'
				load(F)
				list(	THETA= names(fit$bestfit$par),
...
cr.master.ex3.runcoalreg.using.TYPE.ETFI<- function()
{
	...
	infiles[, {
				...
				set(phi, NULL, 'TAXA', NULL)
				tmp		<- data.matrix(phi)
				fit 	<- trf.lasso(dph, 	tmp, trf_names = c( 'TYPE'), aoi_names = c( 'ETSI_NOISE' ), lasso_threshold=5, method = 'BFGS',
						lnr0 = -3, lnrLimits = c(-Inf, -1), scale=FALSE)	
				fci 	<- fisher.ci(fit)	 
				pci 	<- prof.ci(fit, fci  ) 
				#print(fit$bestfit$par )
