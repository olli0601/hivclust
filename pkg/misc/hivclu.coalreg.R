...
cr.various.master<- function()
{
	...
	cr.various.master.ErikBayes()
}

cr.various.master.ErikBayes<- function()
{
	require(coalreg)
	require(ape)
	require(coda)
	...
	if(1)
	{
		...
		trnum <- 1
		outfile <- gsub('\\.nwk',paste0('_output_tree',trnum,'_fullsampling.rda'),infile)
		...
		f6 <-  coreg.adaptiveMHwithinGibbs.or170919(X, tr, ~I0, ~I0
...
cr.master.ex3.adMCMC.evaluate.170925b<- function()
{
	require(data.table)
	require(coda)
	require(ggplot2)
	
	indir	<- '~/Box Sync/OR_Work/2017/2017_coalregression/master_results_8'
	infiles	<- data.table(F=list.files(indir, pattern='^m3.*rda$',full.names=TRUE))
	infiles[, REP:= as.numeric(gsub('.*_rep([0-9]+)_.*','\\1',basename(F)))]
	infiles[, FORMULA_INF:= gsub('.*_inf([A-Z\\+]+)_.*','\\1',basename(F))]
	infiles[, FORMULA_TR:= gsub('.*_tr([A-Z]+)_.*','\\1',basename(F))]
	...
	ggplot(subset(dfm, FORMULA_INF=='ETSI'), aes(x=REP)) + 
			...
			geom_line(data=tmp, aes(x=REP, y=TRUTH), colour='red') +
			facet_grid(variable~SAMPLING+MAXCLADE+MAXHEIGHT+INFPRIOR_S, scales='free')
	ggsave(file=file.path(indir,'boxplots_trTYPE_infETSI_maxheight10_hetinf0_vary_infpriorsd.pdf'),w=10,h=10)
	...
	ggplot(subset(dfm, FORMULA_INF=='TYPE'), aes(x=REP)) + 
			...
			geom_line(data=tmp, aes(x=REP, y=TRUTH), colour='red') +
			facet_grid(variable~SAMPLING+MAXCLADE+MAXHEIGHT+INFPRIOR_S, scales='free')
	ggsave(file=file.path(indir,'boxplots_trTYPE_infTYPE_maxheight10_hetinf0_vary_infpriorsd.pdf'),w=10,h=10)		
}
