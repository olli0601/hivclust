...
seattle.various<- function()
{
	...
	seattle.191017.phydyn.olli.SITmf01yrsXXv3.mle()
}
...
seattle.191017.phydyn.olli.SITmf01yrsXX.assess <- function()
{
	...
	indir <- file.path(home,'phydyn_olli','olli_SITmf01yrsXX_mle')
	...
	df[, SIM_T1:= paste0('length of epidemic ',SIM_T1,' yrs')]
	...
	ggplot(df, aes(x=SIM_YRS, fill=interaction(SIM_RES,SIM_STEP_SIZE_RES), y=abs((ll_fit-ll_sim)/ll_sim) )) +
			geom_boxplot() +
			theme_bw() +
			facet_grid(SIM_T1~SIM_ID, scales='free_y') +
			labs(fill='colik res.\ncolik step_size_res', x='proportion of epidemic sampled at end')
	ggsave(file= file.path(indir,'runs_convergence_lldifference.pdf'), w=9, h=9)
	
	ggplot(df, aes(x=MRAE_LL, y=MAE_PARS, colour=interaction(SIM_RES,SIM_STEP_SIZE_RES))) + geom_point() +
			theme_bw() +
			scale_y_log10() +
	...
	ggplot() + 			
			geom_jitter(data=subset(dfe, SIM_YRS=='40%'), aes(x=SIM_RES, y=value, colour=SIM_RES, shape=convergence), width=1/(4+1), height=0) +
			geom_boxplot(data=subset(dfe, SIM_YRS=='40%'), aes(x=SIM_RES, y=value, group=SIM_RES), fill='transparent', outlier.shape = NA) +
			facet_grid(SIM_ID+SIM_STEP_SIZE_RES+SIM_T1~VAR, scales='free_y') +
			coord_cartesian(ylim=c(-10,10)) +
			theme_bw() +
			geom_hline(data=subset(dft, SIM_YRS=='40%'), aes(yintercept=value)) +
			labs(x='colik res', y='parameter value\n', colour='proportion of epidemic sampled at end', shape='ML optim converged') +
			theme(legend.position='top')
	ggsave(file= file.path(indir,'accuracy_marginals_simyrs40.pdf'), w=10, h=25)
}
...
seattle.191017.phydyn.olli.SITmf01yrsXXv3.mle <- function()
{
	...
	home <- '/rds/general/project/ratmann_seattle_data_analysis/live'	
	simdir <- file.path(home,'phydyn_olli','olli_SITmf01yrsXXv3_sim')
	outdir <- file.path(home,'phydyn_olli','olli_SITmf01yrsXXv3_mle')
	simfiles <- data.table(FIN=list.files(simdir, pattern='rda$', full.names=TRUE))
	simfiles <- subset(simfiles, grepl('^sim[0-9]',basename(FIN)) & grepl('_dettree_|_stotree_',basename(FIN)))
	tmp <- 'sim([0-9])_t1([0-9]+)_yrs([0-9]+)_([a-z]+)_sample([0-9]+)_([0-9]+).(rda|rds)'
	simfiles[, SIM_ID:= as.integer(gsub(tmp,'\\1', basename(FIN)))]
	simfiles[, SIM_T1:= as.integer(gsub(tmp,'\\2', basename(FIN)))]
	simfiles[, SIM_YRS:= as.integer(gsub(tmp,'\\3', basename(FIN)))]
